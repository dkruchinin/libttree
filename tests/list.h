 ...
 * 02111-1307, USA.
 *
 * Doubly-linked list based on include/linux/list.h taken from linux kernel.
...
#include <nidhogg/cdefs.h>
#include <nidhogg/types.h>
...
#define list_node_next_isbound(node)            \
    ((node)->next != NULL)
#define list_node_prev_isbound(node)                                \
    ((node)->prev != NULL)
 ...
 */
static __inline void list_add_range(struct list_node *first, struct list_node *last,
...
/* for internal usage */
static __inline void __list_del_range(struct list_node *first, struct list_node *last)
{
 ...
 */
static __inline void list_del_range(struct list_node *first, struct list_node *last)
{
...
static __inline void list_move(struct list_node *prev,
                               struct list_node *next, struct list_head *from)
{
 ...
 */
static __inline void list_replace(struct list_node *old, struct list_node *new)
{
    old->prev->next = new;
    new->prev = old->prev;
    new->next = old->next;
